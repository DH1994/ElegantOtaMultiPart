...
void ElegantOTAClass::begin(ELEGANTOTA_WEBSERVER *server, const char * username, const char * password){
  ...
  _otaPrefs.begin("otaPrefs", false);
    ...
    _server->on("/ota/start", HTTP_GET, [&](AsyncWebServerRequest *request) {
      ...
      #elif defined(ESP32)  
        if (!Update.begin(UPDATE_SIZE_UNKNOWN, mode == OTA_MODE_FILESYSTEM ? U_SPIFFS : U_FLASH, -1, LOW, NULL, _updatePart)) {
          ELEGANTOTA_DEBUG_MSG("Failed to start update process\n");
          // Save error to string
          StreamString str;
          Update.printError(str);
  ...
  #else
    _server->on("/boot", HTTP_GET, [&]() {
      if (_server->hasArg("partSelect")) {
        String part = _server->arg("partSelect");
        _otaPrefs.putString("bootPart", part);
        bootPartition(part);
      }
    ...
    _server->on("/ota/start", HTTP_GET, [&]() {
      ...
      if (_server->hasArg("part")) {
        _updatePart = _server->arg("part");
        _otaPrefs.putString("bootPart", _updatePart);
      }
      ...
      #elif defined(ESP32)  
        if (!Update.begin(UPDATE_SIZE_UNKNOWN, mode == OTA_MODE_FILESYSTEM ? U_SPIFFS : U_FLASH, -1, LOW, _updatePart.c_str())) {
          ELEGANTOTA_DEBUG_MSG("Failed to start update process\n");
          // Save error to string
          StreamString str;
          Update.printError(str);
...
void ElegantOTAClass::bootPartition(String partStr){
  const esp_partition_t *bootPart;
  if (partStr == "app1") {
    bootPart = esp_partition_find_first(ESP_PARTITION_TYPE_APP, ESP_PARTITION_SUBTYPE_APP_OTA_1, partStr.c_str());
    esp_ota_set_boot_partition(bootPart);
    ELEGANTOTA_DEBUG_MSG(partStr);
  } else if (partStr == "app2") {
    bootPart = esp_partition_find_first(ESP_PARTITION_TYPE_APP, ESP_PARTITION_SUBTYPE_APP_OTA_2, partStr.c_str());
    esp_ota_set_boot_partition(bootPart);
    ELEGANTOTA_DEBUG_MSG(partStr);
  } else if (partStr == "app3") {
    bootPart = esp_partition_find_first(ESP_PARTITION_TYPE_APP, ESP_PARTITION_SUBTYPE_APP_OTA_3, partStr.c_str());
    esp_ota_set_boot_partition(bootPart);
    ELEGANTOTA_DEBUG_MSG(partStr);
  } else {
    return;
    ELEGANTOTA_DEBUG_MSG("Unknown partition\n");
  }
  esp_restart();
}
...
void ElegantOTAClass::loop() {
  if (_boot_timeout && millis() - _boot_timeout_millis > 60000) {
    if(_otaPrefs.isKey("bootPart")) {
      String bootPart = _otaPrefs.getString("bootPart"); 
