...
bool UpdateClass::begin(size_t size, int command, int ledPin, uint8_t ledOn, const char *label) {
  ...
  if (command == U_FLASH) {
    if (label != NULL) {
      log_d("label: %s", label);
      if (strcmp(label, "app0") == 0) {
        _partition = esp_partition_find_first(ESP_PARTITION_TYPE_APP, ESP_PARTITION_SUBTYPE_APP_OTA_0, label);
      } else if (strcmp(label, "app1") == 0) {
        _partition = esp_partition_find_first(ESP_PARTITION_TYPE_APP, ESP_PARTITION_SUBTYPE_APP_OTA_1, label);
      } else if (strcmp(label, "app2") == 0) {
        _partition = esp_partition_find_first(ESP_PARTITION_TYPE_APP, ESP_PARTITION_SUBTYPE_APP_OTA_2, label);
      } else if (strcmp(label, "app3") == 0) {
        _partition = esp_partition_find_first(ESP_PARTITION_TYPE_APP, ESP_PARTITION_SUBTYPE_APP_OTA_3, label);
      } else {
